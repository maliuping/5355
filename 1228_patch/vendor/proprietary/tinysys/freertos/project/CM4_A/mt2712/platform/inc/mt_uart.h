...
#endif
#else
#define UART_SRC_CLK          (6000000) //use in FPGA
#define UART_LOG_BAUD_RATE    (460800)
#endif

#ifdef CFG_SCP_UART1_SUPPORT
#define UART_LOG_PORT        UART4_BASE
#else
#define UART_LOG_PORT        UART4_BASE
#endif

#define UART_SET_BITS(REG, BS)       writel(readl(REG) | (unsigned int)(BS), REG)
#define UART_CLR_BITS(REG, BC)       writel(readl(REG) & ~((unsigned int)(BC)), REG)
